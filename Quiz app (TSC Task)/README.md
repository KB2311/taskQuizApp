...
- follow installation steps from [postcss](https://github.com/webpack-contrib/postcss-loader)
...
### Things To Understand For Project setup and ESlint

- **eslint** is useful for show us error on without runnig like syntax erro or any error in our file also it provide proper way of writing code

- **prettier-eslint-config** help to prevent the miss match of eslint and prettier formate

- **babel parser** is use for if new babel(js rules) introduce than automatic add that rules also in our project

- **husky** is use for prevent error code in Commit of code if our code has error than it not commit the code
